...
(defn primary-keys
  ...
  @(sql/select (schema-db db) [:key-column-usage.*]
     ...
     (sql/join :information-schema.key-column-usage
               '(on (and (= :table-constraints.constraint-name
                            :key-column-usage.constraint-name)
                         (= :table-constraints.constraint-type
                            "PRIMARY KEY"))))
     (sql/where `(and (= :key-column-usage.table-schema
                         ~(table-schema db identifier))
                      (= :key-column-usage.table-name
                         ~(table-name db identifier))
                      (= :table-constraints.constraint-type
                         "PRIMARY KEY")))
...
(defn unique-keys
  ...
  [db identifier]
  @(sql/select (schema-db db) [:information-schema.table-constraints.* :column-name]
     (sql/from :information-schema.table-constraints)
     (sql/join :information-schema.key-column-usage.constraint-name
               :information-schema.table-constraints.constraint-name)
     (sql/where `(and (= :table-constraints.table-schema ~(table-schema db identifier))
...
(defn column
  ...
  (let [{:keys [schema table] :as column} (expr/parse-column identifier)]
    (first @(sql/select (schema-db db) [:columns.*]
              ...
              (sql/where `(and (= :table-schema ~(or (some-> schema name) "public"))
...
(defn columns
  ...
  @(sql/select (schema-db db) [:columns.*]
     (sql/from :information-schema.columns)
     (sql/where `(and (= :table-schema ~(table-schema db identifier))
...
(defn table
  ...
  [db identifier]
  (let [primary-keys (primary-keys db identifier)
        unique-keys (unique-keys db identifier)
        columns (columns db identifier)]
