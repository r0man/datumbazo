...
(deftest test-define-instance?
  (is (= (#'record/define-instance? continents-table)
...
(deftest test-define-make-instance
  (is (= (#'record/define-make-instance continents-table)
...
(deftest test-define-find-all
  (is (= (#'record/define-find-all continents-table)
...
(deftest test-on-conflict-clause
  (is (= (#'record/on-conflict-clause Continent)
         [:id])))

(deftest test-do-update-clause
  (is (= (#'record/do-update-clause Continent)
...
(deftest test-primary-key-columns
  (is (= (#'record/primary-key-columns Continent)
         #{{:schema nil,
            :children [:name],
            :table :continents,
            :primary-key? true,
...
(deftest test-unique-key-columns
  (is (= (#'record/unique-key-columns Continent)
         #{{:schema nil,
            :children [:name],
            :not-null? true,
            :table :continents,
...
(deftest test-select-class
  (is (= (sql/sql (record/select-class db Continent))
