...
(deftest test-deserialize-record
  (let [language (deserialize-record :wikipedia.languages {:name "German" :iso-639-1 "DE" :iso-639-2 "DEU"})]
    (is (map? language))
...
(deftest test-serialize-record
  (let [language (serialize-record :wikipedia.languages {:name "German" :iso-639-1 "DE" :iso-639-2 "DEU" :not-existing "column"})]
    (is (map? language))
    (is (= "German" (:name language)))
    (is (= "de" (:iso-639-1 language)))
    (is (= "deu" (:iso-639-2 language)))
